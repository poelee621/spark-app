...
严格只返回一个 JSON 对象，不要任何解释或 markdown 代码块：
{
  ...
  "cardLines": [
    "第1张大字封面短标题（必须填，最多3行，每行最多8个字）",
    "第2张痛点提炼短句（必须填，最多3行，每行最多8个字）",
    "第3张干货提炼短句（必须填，最多3行，每行最多8个字）",
    "第4张金句（必须填，最多3行，每行最多8个字）"
  ],
  "cardLines约束": "4 项都必须填写且禁止为空；每张最多 3 行、每行最多 8 个汉字，超出会被裁切；用短文案而非长句；按 style 的语气写（犀利用锋芒短句、温情用共鸣短句、干货用步骤短句等）"
}
