...
严格只返回一个 JSON 对象，不要任何解释或 markdown 代码块：
{
  ...
  "cardLines": [
    "第1张大字封面短标题（必须填，建议1-2行、每行4-6个字为佳，最多3行/每行最多8个字）",
    "第2张痛点提炼短句（必须填，建议1-2行、每行4-6个字为佳，最多3行/每行最多8个字）",
    "第3张干货提炼短句（必须填，建议1-2行、每行4-6个字为佳，最多3行/每行最多8个字）",
    "第4张金句（必须填，建议1-2行、每行4-6个字为佳，最多3行/每行最多8个字）"
  ],
  "cardLines约束": "4 项都必须填写且禁止为空；用短词组/短句，每行4-6字最佳、最多8字；每张最多3行；严禁出现逗号/句号隔开的完整长句；按 style 语气写（犀利=锋芒短词、温情=共鸣短词、干货=动作短词）"
}
